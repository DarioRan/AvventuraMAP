...
public class DBManager {
    ...
    public GameStatus getGameStatus(final String username) throws SQLException {
        ...
        if (rs.next()) {
            ...
            return new GameStatus(username, Integer.valueOf(lastRoomId), inventoryIds, time.toLocalDateTime());
        }
        return null;
    ...
    public int insertNewGameStatus(final GameStatus gamestatus) throws SQLException {
        ...
        if (rs.next()) {
            query = "UPDATE GameStatus SET LastRoom = '" + gamestatus.getLastRoomId().toString() + "', Inventory = '" + gamestatus.getInventoryIdsAsString() + "' WHERE username = '" + gamestatus.getUsername() + "'";
